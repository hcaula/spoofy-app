...
class App extends Component {
    ...
    componentDidMount() {
        this.login()
            .then(res => {
                this.setState({ user: res.user, access_token: res.access_token, ready: true });
            })
    ...
    render() {
        let div;
        if (this.state.user) div = <Dashboard logout={this.logout} user={this.state.user} token={this.state.access_token} />
        else div = <Home />
